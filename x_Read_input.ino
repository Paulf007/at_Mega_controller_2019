...
void  reportInputStates_old() {
  for (int thisPin = 0; thisPin < buttcount; thisPin++) {
    char* state = inputStates[thisPin] == HIGH ? OFF  : ON ;
    publishPinState(thisPin , state);
...
// Read Inputs and Report States
  for (int i = 0; i < NUM_BUTTONS; i++)  {
    ...
    if ( buttons[i].fell() ) {
      //Serial.print(F("Button Link Fell:"));
      //Serial.println(i);
      char stopic[40];
  ...
  //   needToToggleLed = true;
      ...
      char stopic[40];
      //Serial.print(F("Button Link Rose:"));
      //Serial.println(i);
      sprintf(stopic ,"%s%d",outPinTopicV,i);
...
void connect_relay_test (int swpin) {                  // void switchRelay(char* switchState, int pos)
